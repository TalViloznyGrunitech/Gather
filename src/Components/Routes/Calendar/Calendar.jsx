...
export default function Calendar() {
  ...
  const calendarEvents = useMemo(() => {
    ...
    return joinedEvents
      .map((event) => {
        try {
          // Try to match full datetime format first: "2024-01-15 at 2:00 PM"
          let dateTimeMatch = event.dateTimeLabel.match(
          ...
          if (dateTimeMatch) {
            ...
            // Try to match just date format: "2024-01-15"
            const justDateMatch =
              event.dateTimeLabel.match(/\d{4}-\d{2}-\d{2}/);

            if (justDateMatch) {
              // Just date format - set to 12:00 PM by default
              eventDate = new Date(justDateMatch[0]);
              eventDate.setHours(12, 0, 0, 0);
              console.log(
                "Parsed date-only event:",
                event.title,
                justDateMatch[0]
              );
            } else {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    if (eventId.startsWith("test-event")) {
      eventData = {
        ...
        categoryClassName:
          eventId === "test-event-1" ? "TechnologyCategory" : "SportsCategory",
      };
    } else {
      ...
      <div className="Main">
        <div className="CalendarContainer">
          <div className="CalendarHeader">
            <h1>No Calendar</h1>
            <p>
              <span>⛔</span>Error: Please log in to view your events calendar.
            </p>
          </div>
        </div>
      </div>
